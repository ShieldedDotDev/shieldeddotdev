...
| `/api/user/tokens` | GET | Returns the current user's API-token metadata. Plaintext token values and hashes are never returned. |
| `/api/user/tokens` | POST | Creates a user-level API token from a required description. Returns its metadata and plaintext token once with `201 Created`. |
| `/api/user/tokens/{id}` | DELETE | Revokes an owned user-level API token and returns `204 No Content`. |
...
- `Home` calls `/env`, mounts API usage examples, and renders the Preact static badge generator on the home page.
